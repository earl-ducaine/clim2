...
(ff-wrapper:defun-foreign-callable match-event-sequence-and-types-using-structure
    ((display :foreign-address)
     ...
     (arg :foreign-address))
  (let* ((desired-display (ff-wrapper:fslot-value-typed 'event-match-info :c arg 'display))
	 (desired-sequence (ff-wrapper:fslot-value-typed 'event-match-info :c arg 'seq-no))
         (n-types (ff-wrapper:fslot-value-typed 'event-match-info :c arg 'n-types))
