...
;;; The user entry points are walk-form and nested-walked-form.  In
;;; addition, it is legal for user code to call the variable
;;; information functions: variable-lexical-p, variable-special-p and
;;; variable-class.  Some users will need to call
;;; define-walker-template, they will have to figure that out for
;;; themselves.
...
;;;
;;; On the following pages are implementations of the implementation
;;; specific environment hacking functions for each of the
;;; implementations this walker has been ported to.  If you add a new
;;; one, so this walker can run in a new implementation of Common
;;; Lisp, please send the changes back to us so that others can also
;;; use this walker in that implementation of Common Lisp.
;;;
;;; This code just hacks 'macroexpansion environments'.  That is, it
;;; is only concerned with the function binding of symbols in the
...
;;; bindings.  It must be able, when walking a macrolet, flet or
;;; labels form to construct an environment which reflects the
;;; bindings created by that form.  Note that the environment created
;;; does NOT have to be sufficient to evaluate the body, merely to
;;; walk its body.  This means that definitions do not have to be
;;; supplied for lexical functions, only the fact that that function
;;; is bound is important.  For macros, the macroexpansion function
;;; must be supplied.
;;;
...
;;; environments after the body returns.  Other user code is free to
;;; do so in implementations where it works, but that code is not
;;; considered portable.
;;;
;;; There are 3 environment hacking tools.  One macro which is used
;;; for creating new environments, and two functions which are used to
;;; access the bindings of existing environments.
;;;
...
;;; In Coral Common Lisp, the macroexpansion environment is just a
;;; list of environment entries. The cadr of each element specifies
;;; the type of the element. The only types that interest us are
;;; CCL::MACRO and FUNCTION. In these cases the element is interpreted
;;; as follows.
;;;
