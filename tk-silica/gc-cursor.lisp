...
(defun init-gc-cursor (frame &optional force)
  ...
  (let ((pointer-type (first
                       (list #-allegro  '(unsigned-byte 64)
                             ;; otherwise, use 32 bits:
                             #+allegro '(unsigned-byte 32)))))
      ...
      (when (or force (null *gc-before*))
        (let ((vec (make-array 2 :element-type pointer-type)))
          ...
          (setq *gc-before* (aref vec 0)
                *gc-after*  (aref vec 1))
          #+allegro (pushnew (make-array 1 :element-type pointer-type
...
#+allegro (pushnew 'reinitialize-gc-cursor excl::*restart-actions*)
